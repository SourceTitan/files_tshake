...
database:set('tshake:'..bot_id..'SH:BOT'..msg.chat_id_,true)
return false
end
if text == 'تعطيل تنبيه الاشعار' then
send(msg.chat_id_, msg.id_, 1, '📮┇ تم تعطيل التنبيه عل شعار  !', 1, 'md')
database:del('tshake:'..bot_id..'SH:BOT'..msg.chat_id_)
return false
end
if text and text:match("^وضع الشعار (.*)$") then
local SHGP = text:match("^وضع الشعار (.*)$")
database:set('tshake:'..bot_id..'SH:BOT:GP'..msg.chat_id_,SHGP)
send(msg.chat_id_, msg.id_, 1, '📮┇تم حفظ شعار المجموعه \n', 1, 'md')
end
end
if text and database:get('tshake:'..bot_id..'SH:BOT'..msg.chat_id_) and not is_devabas(msg) and not is_sudo(msg) then
if tonumber(database:get('tshake:'..bot_id..'SH:NUM'..msg.chat_id_..msg.sender_user_id_) or 0) > 3 then
chat_kick(msg.chat_id_, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📮┇ تم طردك من المجموعة لعدم احترام قوانين المجموعة', 1, 'md')
database:del('tshake:'..bot_id..'SH:NUM'..msg.chat_id_..msg.sender_user_id_)
else
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
local last_ = data.last_name_ or ''
local first_ = data.first_name_ or ''
local taha = (first_..''..last_)
local taha1 = (database:get('tshake:'..bot_id..'SH:BOT:GP'..msg.chat_id_) or '')
if string.find(taha,taha1) == nil then
local taha = tonumber((database:get('tshake:'..bot_id..'SH:NUM'..msg.chat_id_..msg.sender_user_id_) or 0))
if taha == 1 then
...
database:incrby('tshake:'..bot_id..'SH:NUM'..msg.chat_id_..msg.sender_user_id_,1)
end
end,nil)  
return false
end
